<template>
  <div>
    <div>
      <el-button v-if="showAdd" :disabled="editFlag" icon="el-icon-plus" type="primary" size="small" @click="add">添加</el-button>
      <el-popover
        ref="delPopover"
        placement="top"
        width="190"
        v-model="batchDelete">
        <p><i class="el-icon-warning" style="color:#e6a23c;margin-right: 7px"></i>是否删除信息？</p>
        <div style="text-align: right;margin-top: 10px;">
          <el-button size="mini" type="text" @click="batchDelete = false">取消</el-button>
          <el-button type="primary" size="mini" @click="dbDel">确定</el-button>
        </div>
      </el-popover>
      <el-button :disabled="editFlag" size="small" type="danger" icon="el-icon-delete" @click="batchDelete=true"
                 v-popover:delPopover
                 v-if="showDel && multipleSelection.length>0&&options.hasDbDel">批量删除
      </el-button>
          ...
          <template slot-scope="scope">
            <div v-show="scope.row.options.isEditing">
              <el-select ref="select" v-if="item.type=='select' && item.isBlurry"
                         ...
                         placeholder="请选择" :filter-method="selectBlurry"
                         size="small">
              ...
              <el-input v-if="item.type=='input'" v-model="scope.row.options.value[item.prop]" size="small" :maxlength="item.maxlength"></el-input>
              <el-input v-if="item.type=='textarea'" type="textarea" v-model="scope.row.options.value[item.prop]" size="small" :autosize="{ minRows: 2, maxRows: 4}"></el-input>
              <div v-if="!item.type">
            ...
            <div v-show="!scope.row.options.isEditing">
              ...
              <span v-else>{{scope.row[item.prop]=='' || scope.row[item.prop] == null ? '-' : scope.row[item.prop]}}</span>
            </div>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div v-show="!scope.row.options.isEditing">
              <el-button :disabled="editFlag" type='text' size="mini" @click="edit(scope)"
                         v-if="showEdit && !scope.row.options.noEdit">编辑
              </el-button>
              <span v-if="showEdit &&!scope.row.options.noEdit" style="color:#e8e8e8 !important; ">|</span>
              <el-popover
                placement="top"
                ref="popoverDel"
                width="190"
                v-model="scope.row.delFlag">
                <p><i class="el-icon-warning" style="color:#e6a23c;margin-right: 7px"></i>是否删除该批次？</p>
                <div style="text-align: right;margin-top: 10px;">
              ...
              <el-button v-if="showEdit" :disabled="editFlag" v-popover:popoverDel type='text' size="mini" slot="reference"
        ...
        if (scope.row.options.isNew) {
          ...
          if (res.flag) {
            for (let i in this.tableData) {
              if (this.tableData[i].id == res.data.id) {
                for (let j in scope.row) {
                  for (let k in res.data) {
                    if (j == k) {
                      scope.row[j] = res.data[k]
                    }
                  }
                }
              }
