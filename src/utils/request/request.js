...
let request = async (options) => {
  ...
  let x_docment_token = getToken('x-docment-token')
  if(x_docment_token){
    options['headers'] = merge(options['headers'], {"x-docment-token": x_docment_token});
  }
  ...
  try {
    ...
    const {statusText, status, data} = result
     if (typeof window.bus !== 'undefined' && window.bus.$Message) {
      if ( data && data.code && data.code !== '00000' &&  data.code !== 1) {
